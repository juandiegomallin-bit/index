...
/* ════════════════════════════════════════════════════════════
   PARTE 4.5 — ACCESO RÁPIDO (CUENTAS DE DEMOSTRACIÓN)
   Los botones "⚡ Acceso rápido" del login llaman a esta función
   mediante onclick="llenarCredenciales(correo, contraseña)".
   Antes no existía, por eso los botones no hacían nada.
════════════════════════════════════════════════════════════ */

/**
 * Autocompleta los campos de correo y contraseña del formulario
 * de login con una de las cuentas de demostración, limpia los
 * errores/alertas previos y da una pequeña señal visual.
 * @param {string} correoDemo - Correo de la cuenta de prueba
 * @param {string} passDemo   - Contraseña de la cuenta de prueba
 */
function llenarCredenciales(correoDemo, passDemo) {
    emailInput.value = correoDemo;
    passInput.value  = passDemo;

    // Limpia mensajes de error y alertas que pudieran estar visibles
    emailError.textContent = '';
    passError.textContent  = '';
    loginAlert.className   = 'alerta';
    loginAlert.textContent = '';

    // Resalta brevemente los campos para confirmar que se llenaron
    [emailInput, passInput].forEach(input => {
        input.style.borderColor = 'var(--verde-claro)';
        setTimeout(() => { input.style.borderColor = ''; }, 700);
    });

    // Lleva el foco al botón de enviar para agilizar el acceso
    document.querySelector('#loginForm button[type="submit"]').focus();
}

// Se expone en window porque el HTML la invoca desde atributos onclick
...
function limpiarAlertas() {
    ...
    loginAlert.className    = 'alerta';
    loginAlert.textContent  = '';
    registerAlert.className = 'alerta';
...
form.addEventListener('submit', function(e) {
    ...
    if (usuario) {
        ...
        loginAlert.className    = 'alerta success';
        ...
        loginAlert.className   = 'alerta error';
...
registerForm.addEventListener('submit', function(e) {
    ...
    registerAlert.className   = 'alerta success';
